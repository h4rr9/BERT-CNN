...
class RTEDataLoader(BaseDataLoader):

    def __init__(self, config):
        super(RTEDataLoader, self).__init__(config)

        train = pd.read_csv(os.path.join(
            _DATA_PATH, "RTE", "train.tsv"), sep="\t")
        val = pd.read_csv(os.path.join(
            _DATA_PATH, "RTE", "dev.tsv"), sep="\t")
        test = pd.read_csv(os.path.join(
            _DATA_PATH, "RTE", "test.tsv"), sep="\t")
