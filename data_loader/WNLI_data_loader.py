...
class WNLIDataLoader(BaseDataLoader):

    def __init__(self, config):
        super(WNLIDataLoader, self).__init__(config)

        train = pd.read_csv(os.path.join(
            _DATA_PATH, "WNLI", "train.tsv"), sep="\t")
        val = pd.read_csv(os.path.join(
            _DATA_PATH, "WNLI", "dev.tsv"), sep="\t")
        test = pd.read_csv(os.path.join(
            _DATA_PATH, "WNLI", "test.tsv"), sep="\t")
        ...
        self.train_dataset = data.convert_examples_to_features(
            tokenizer, train_examples, max_seq_length=256)
        self.val_dataset = data.convert_examples_to_features(
            tokenizer, val_examples, max_seq_length=256)
        self.test_dataset = data.convert_examples_to_features(
            tokenizer, test_examples, max_seq_length=256)
